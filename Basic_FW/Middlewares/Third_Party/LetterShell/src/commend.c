...
void var(int argc, char** argv)
{
    ...
    if (argc > 4)//如果传入数量为4--有错误
		{
        logInfo("paramer error\r\n");
        return;
    }

    if(0==strcmp(argv[1], "all"))//如果第二个字符串的值是all
		{
    ...
    while (0!=strcmp(argv[1], _var->name))//循环判断输入是否等于关键字
    {
    ...
    if(argc==4)//找到之后进行判断，传入数量是否为4
		{

        if(strcmp(argv[2],"=")==0){//第三个传入的变量是否为整数
                if(argc==4){
                    _var_second=_var_begin;
                    while (0!=strcmp(argv[3], _var_second->name))//链表结构判断关键字
                    {
    ...
    switch (_var->type)//现在对关键字进行匹配
    {

    case _8://如果是8位应该打印你输入的对应的值，以下都是
...
//	if(argc==1)//如果参数是1 查找所有的配置项
//    {
//       fdb_kv_print(&kvdb);//把
//       return 0;
// 
//    }
//	if(argc==2)//如果只有两个就查找，如果没有就直接创建，默认赋值为0
//	{
//		//第二个参数是键，第一个参数是句柄，第三个参数是值（这里是创建）（第一个是句柄，第二个是值，第三个值的大小）
...
int DB_Co_API(int argc,char* name, char *KV)//name键 KV 值
{
			float num=0.0;
			struct fdb_blob blob;
			extern struct fdb_kvdb kvdb;
